...
create(ClusterSpec,Lock)->
    gen_server:call(?SERVER, {create,ClusterSpec,Lock},infinity).
...
%% @end
%%--------------------------------------------------------------------
-spec get_lock() -> {ok,Lock :: atom()} | {error, Error :: term()}.

get_lock()->
    gen_server:call(?SERVER, {get_lock},infinity).

%% Get the creator node  
%% @end
%%--------------------------------------------------------------------
-spec delete() -> ok | {error, Error :: term()}.
...
init([]) ->
    ...
    ok=lib_etcd_paas_config:create_table(),
...
handle_call({get_info}, _From, State) ->
    Reply=lib_etcd_paas_config:get_info(),
    {reply, Reply, State};


handle_call({get_cluster_spec}, _From, State) ->
    Reply=lib_etcd_paas_config:get(cluster_spec),
    {reply, Reply, State};

handle_call({get_lock}, _From, State) ->
    Reply=lib_etcd_paas_config:get(lock),
    {reply, Reply, State};

handle_call({create,ClusterSpec,Lock}, _From, State) ->
    Reply=lib_etcd_paas_config:create(ClusterSpec,Lock),
    {reply, Reply, State};
handle_call({delete}, _From, State) ->
    Reply=lib_etcd_paas_config:delete(),
    {reply, Reply, State};
